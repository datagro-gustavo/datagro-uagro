...
const Clean = styled.div`
    height:${props => props.scrolled ? "2.2rem" : "0"};
...
const BannerEl = styled.img`
    margin-top: 1.5rem;
`
export const SectionArtigo = ({ id, columnistId, slug }) => {
    ...
    const { columnist, getColumnistById, getColumnistHistorySection } = useContext(ColumnContext)
    ...
    const onClickGoToCategory = (id, item) => {
        ...
        window.location.href = `/cultura/${id}/${slug}`
    };
    ...
    useEffect(() => {
        (async () => {
            if (slug) {
                await getByHref(slug)
                await getColumnistById(columnistId)
            } else {
                await getById(id)
                await getColumnistById(columnistId)
            }
    ...
    useEffect(() => {
        getColumnistHistorySection(columnistId)
    }, [])
    ...
    const novaUrl = notice?.imageUrl?.replace(
        'https://assets.datagro.com/wp-content/uploads//',
    ...
    return (
        <main className={`${theme ? "bg-white text-black" : "bg-black text-[#FFFFFF]"} min-h-screen flex flex-col justify-center`}>
            ...
            <NavBar props={'max-w-7xl mx-auto px-0'} />
            <ModalSign />
            <BackgroundModal />
            <motion.main
            ...
            >
                <section className="flex max-w-7xl mx-auto xl:max-w-[1280px] 2xl:max-w-[1650px] px-6 1280px py-7 ">
                    <div className={String(notice?.htmlContent).length >= 1300 ?
                            ...
                            <CardAuthor>
                                ...
                                {animated
                                    ...
                                    <>
                                        ...
                                        <p className="font-metropolis text-sm "><b>Por:</b></p>
                                    </>
                            ...
                            <RowItems>
                                ...
                                <BoxItem>
                                    <div className="flex gap-2 ">
                                        <Image width={26} height={26} src={facebookIcon} onClick={() => shareFacebook()} className="w-[26px]" />
                                        <Image width={26} height={26} src={twitterIcon} onClick={() => shareTwitter()} className="w-[26px]" />
                                        <Image width={26} height={26} src={linkedinIcon} onClick={() => shareLinkedin()} className="w-[26px]" />
                                        {/* <img src={youtubeIcon} onClick={() => shareYouTube()} className="w-[26px]" /> */}
                                        <Image width={26} height={26} src={whatsIcon} onClick={() => shareWhatsApp()} className="w-[26px]" />
                                        <Image width={26} height={26} src={linkIcon} onClick={() => copyLink()} className="w-[27px]" />
                                    </div>
                                </BoxItem>
                            </RowItems>
                        </BoxInfo>
                        ...
                        <figure className="mb-7 flex flex-col items-center">
                            {animated
                                ?
                                <AnimatedImage>
                                ...
                                :
                                <img src={novaUrl} alt="foto" className="object-cover w-full h-auto " />
                            }

                        </figure>

                        <Notice notice={notice.htmlContent} />

                        <TechColumnist name={`Outros artigos de ${columnist?.name}`} columnistId={columnistId} mx="0" px="0" />
