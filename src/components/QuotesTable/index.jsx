...
export const QuotesTable = () => {
	...
	const { listQuadrosIds, setQuadroId, quadroData } = useContext(QuadrosContext);
	...
	return (
		<div>
			<div className="flex flex-col sm:flex-row mb-4">
				...
				<div className="flex items-center justify-end w-full relative mt-3 sm:mt-0">
					{/* Caixa de busca */}
					<div
						className={`
              mr-2 sm:mr-4
              ...
              flex items-center
              ${searchBox ? 'scale-x-100 opacity-100 w-full sm:w-72' : 'scale-x-0 opacity-0 w-0'}
            `}
					>
						<div className="relative w-full">
							<input
								...
								className="border border-gray-300 rounded px-3 py-2 w-full text-sm sm:text-base pr-8"
								value={query}
							...
							{(query || (dataQuote && dataQuote.length > 0)) && (
								<div className="absolute left-0 right-0 mt-1 bg-white max-h-60 z-50 overflow-y-auto shadow-lg border border-gray-200 rounded-md">
			...
			<div className="w-full overflow-x-auto">
				<table className="min-w-full bg-white">
					<thead>
						<tr className="uppercase text-xs sm:text-md border-b border-gray-200">
							<th className="py-2 px-4 text-left">Ativo</th>
							<th className="py-2 px-4 text-right">ÚLTIMO</th>
							...
							<th className="py-2 px-4 text-right">Data</th>
							<th className="py-2 px-4 text-right">Hora</th>
						</tr>
					</thead>
					<tbody>
							...
							<tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
								...
								>
									<div className='flex flex-col'>
										<span className='text-sm sm:text-md hover:underline'>{quote.cod}</span>
										<span className='text-xs'>{quote.long}</span>
									</div>
								</td>
								<td className="py-2 px-4 text-right text-7md font-semibold">{quote.value}</td>
								<td className="py-2 px-4 text-right">
									<span className='text-sm'>{quote.max}</span>
								</td>
								<td className="py-2 px-4 text-right">
									<div>
										<span className={` text-sm`}>{quote.min}</span>

									</div>
								</td>
								<td
									className={`py-2 px-4 text-right ${quote.var >= 0 ? 'text-green-600' : 'text-red-600'
										}`}
								>
								...
								<td className="py-2 px-4 text-right">{formatDate(quote.date)}</td>
								<td className="py-2 px-4 text-right">{quote.hour}</td>
							</tr>
						))}
					</tbody>
				</table>
