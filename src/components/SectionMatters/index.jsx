'use client'

import React, { useContext, useEffect, useState } from 'react'
import styled from "styled-components"

/* COMPONENTS */
import { NavBar } from "@/components/NavBar/NavBar"
import CardMoreNotice from "@/components/CardMoreNotice"
import { Footer } from "@/components/Footer"
import { BannersContext } from '@/context/banners'
import { EditoriaAside } from '../EditoriaAside'
import { Sidebar } from '../Sidebar'
import { ModalSign } from '../ModaSign'
import BackgroundModal from '../BackgroundModal'
import Skeleton from 'react-loading-skeleton'
import { MattersContext } from '@/context/matters'
import { Swiper, SwiperSlide } from "swiper/react";
import { Autoplay, Pagination } from "swiper/modules";

import "swiper/css";
import "swiper/css/pagination";

const Border = styled.div`
    @media screen and (max-width:920px){
        display: none;
    }
`

const BannerCarouselWrapper = styled.div`
  width: 100%;
  max-width: 100%;
  margin: 2rem auto;

  .swiper-pagination-bullet {
    background: #ccc;
    opacity: 1;
  }

  .swiper-pagination-bullet-active {
    background: #8CB024;
  }
`;

const BannerCarouselImage = styled.img`
  width: 100%;
  height: auto;
  display: block;
  border-radius: 6px;
  cursor: pointer;
  object-fit: cover;
`;

const SectionNoticesMatters = ({ id, slug, page }) => {
    const [dataMatter, setDataMatter] = useState(null);

    const {
        rowNotices,
        hasMoreRows,
        loadMoreRowNotices,
        setMatter,
        animatedNotices,
        matters
    } = useContext(MattersContext);

    const { get, getBannerPositionInfo } = useContext(BannersContext);

    const bannerPosition = getBannerPositionInfo(5, "matters", dataMatter?.Name);

    const slugify = (text) => {
        return text
            ?.toString()
            .toLowerCase()
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .replace(/\s+/g, "-")
            .replace(/[^\w-]+/g, "")
            .replace(/--+/g, "-")
            .trim();
    };

    useEffect(() => {
        setMatter(id);
    }, [id, slug, setMatter]);

    useEffect(() => {
        const foundMatter = matters.find(item => slugify(item?.Name) === id);

        if (foundMatter) {
            setDataMatter(foundMatter);
        } else {
            setDataMatter(null);
        }
    }, [matters, id]);

    useEffect(() => {
        if (!dataMatter?.Name) return;
        get(5, "matters", dataMatter.Name);
    }, [dataMatter?.Name, get]);

    return (
        <section>
            <NavBar />
            <BackgroundModal />

            <div className="flex flex-col md:flex-row md:px-20 md:justify-center">
                <div className='md:px-0 px-5 md:max-w-[920px]'>

                    <BannerCarouselWrapper>
                        {bannerPosition?.banners?.length > 0 && (
                            <Swiper
                                modules={[Autoplay, Pagination]}
                                slidesPerView={1}
                                spaceBetween={12}
                                loop={bannerPosition.banners.length > 1}
                                autoplay={
                                    bannerPosition.banners.length > 1
                                        ? {
                                            delay: 3000,
                                            disableOnInteraction: false,
                                        }
                                        : false
                                }
                                pagination={{ clickable: true }}
                            >
                                {bannerPosition.banners.map((item) => (
                                    <SwiperSlide key={item.id}>
                                        <BannerCarouselImage
                                            src={item.imageUrl}
                                            alt={item.name || "Banner"}
                                        />
                                    </SwiperSlide>
                                ))}
                            </Swiper>
                        )}
                    </BannerCarouselWrapper>

                    <div className='flex items-center gap-5 py-4'>
                        <div className='w-[5px] h-[79px] bg-[#98BF0E] rounded-[15px]' />
                        <div>
                            <h1 className="text-[1.5rem] mt-0 h-[1.31rem] font-bold text-[#000000]">
                                {dataMatter?.Name}
                            </h1>
                            <h2 className="text-[0.9rem] md:max-w-[420px] mt-6 font-medium text-[#000000]">
                                {dataMatter?.DisplayNames}
                            </h2>
                        </div>
                    </div>

                    {animatedNotices ? <Skeleton /> : null}

                    {rowNotices?.map(item => (
                        <CardMoreNotice
                            key={item?.id}
                            title={item?.title}
                            publishDate={item?.publishDate}
                            matters={item?.matters}
                            markets={item?.markets}
                            description={item?.description}
                            image={item?.imageUrl}
                            noticeId={item?.id}
                            slug={item?.slug}
                        />
                    ))}

                    {hasMoreRows && rowNotices.length > 0 && (
                        <button
                            onClick={() => loadMoreRowNotices()}
                            className="cursor-pointer mt-9 mb-10 px-6 py-2 bg-[#319e96] text-white rounded-md hover:bg-secondary/90 transition-colors"
                        >
                            Ver mais notícias
                        </button>
                    )}
                </div>

                {rowNotices.length === 0 ? <div className='w-[440px]' /> : null}

                <div className='ml-30 flex'>
                    <Border className="hidden mr-15 ml-3 md:flex border border-r-0 ml-0 mr-0 border-slate-500 h-[auto]" />
                    <EditoriaAside matter={dataMatter?.Name} />
                </div>
            </div>

            <Footer />
            <Sidebar />
            <ModalSign />
        </section>
    );
}

export default SectionNoticesMatters;