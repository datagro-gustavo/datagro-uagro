...
const TechSection = ({ mx, px, name, home = 0 }) => {

  const isHome = location.pathname === "/"
  const isNews = location.pathname.startsWith("/news");

  const config = getSessionConfig("tech");
  const { byCategory, setCategory, category, notice, notices } = useContext(NewsContext)
  ...
  const noticeList = Array.isArray(byCategory) ? byCategory : [];
